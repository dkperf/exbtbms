...
#define DBUG(block){if(gpar.printflag==PRINT_VERBOSE){timestamp();block}}
...
int init_blue_ex(char *filename,int hcin)
{
   ...
   //dk set_print_flag(PRINT_VERBOSE);
   DBUG( printf( "Initialising...\n");)
   ...
   lecap = 0;
   flag = 0;
   DBUG( printf( "Read local supported commands\n");)
   sendhci(locsup,0);
   ...
   DBUG( printf( "Read local board address\n");)
   //flushprint();
   sendhci(locbadd,0);
   ...
   {
      printf( "Unable to read local board address\n");      
      errcount = 1000;
   }
   ...
   if(lecap != 0)
   {
      ...
      DBUG( printf( "Set LE advertising parameters\n");)
      sendhci(leadparam,0);
      statusok(0,leadparam);
      DBUG( printf( "Set LE advertising data\n");)
   ...
   if(stream == NULL)
   {
      printf( "Unable to open devices info file %s\n",filename);
      readret = 0;
   }
   else
      readret = readline(stream,s);  // read first line

   //dk  printf( "Device data from %s file\n",filename);
   DBUG( printf( "Device data from %s file\n",filename);)
   //flushprint();
   ...
   while(readret > 0 && errcount < 6)
   {
      ...
      if(flag != 0)
      {
         ...
         {  // TYPE  
            ...
            {
               printf( "%sTYPE must be CLASSIC/LE/MESH\n",errs);
               errflag = 1;
            }
         }  // end ind[1] TYPE
         ...
         {  // ADDRESS 
            ...
            if(hn != 6)
            {
               printf( "%sDevice address must be 6 bytes\n",errs);
               errflag = 1;
            }
            else
            {
         ...
         { // HANDLE
            ...
            if(!(hn == 1 || hn == 2))
            {
               printf( "%sHANDLE must be 1 or 2 bytes\n",errs);
               errflag = 1;
            }
            else
            { 
         ...
         {  // UUID 
            ...
            if(!(hn == 2 || hn == 16))
            {
               printf( "%sUUID must be 2 or 16 bytes\n",errs);
               errflag = 1;
            }
            else
            {
         ...
         {  //PERMIT; 
            ...
            if(hn != 1)
            {
               printf( "%sPERMIT must be 1 hex byte\n",errs);
               errflag = 1;
            }
            else 
               cp->perm = data[0];               
   ...
   if(dev[0]->node == 0)
   {
      ...
      printf( "It should be added to the %s file as follows:\n",filename);
      printf( "DEVICE=this device name  TYPE=MESH  NODE=choose  ADDRESS=%s\n",baddstr(dev[0]->baddr,0));
   }
   else
   {  // write local name
      for(n = 0 ; n < 8 ; ++n)
   ...
   if(lecap == 0)
   {
      ...
      printf( "*** Mesh/LE functions will not work *******\n");
   }
   else
   {
      DBUG( printf( "Set LE advertising data with device name\n");)
      addname();
      sendhci(leadvert,0);  // reset mesh packet index = 0
      statusok(0,leadvert);
   }
...
int localctics()
{
   ...
   for(n = 0 ; ctic(0,n)->type == CTIC_ACTIVE ; ++n)
   {
      ...
      if(cp->chandle != 0)
      {
         if(cp->chandle < 5)
         {
            printf( "%shandle must be 5 or greater\n",errs);
            return(0);
         }
         for(j = 0 ; ctic(0,j)->type == CTIC_ACTIVE ; ++j)
         {
            ...
            if(cpx->chandle != 0 && j != n)
            {
               ...
               {
                  printf( "%shandle %04X interferes with handles used by %s\n",errs,cp->chandle,cpx->name);
                  return(0);
               }
            }
         }
   ...
   for(n = 0 ; ctic(0,n)->type == CTIC_ACTIVE ; ++n)
   {
      ...
      {
         printf( "%sUUID not allowed\n",errs);
         return(0);
      }

      if(cp->size < 1 || cp->size > LEDATLEN)
      {
         printf( "%ssize must be 1-%d\n",errs,n,LEDATLEN);
         return(0);
      }
...
int write_mesh(char *buf,int count)
{
   ...
   if(count < 0 || count > 25)
   {
      printf( "More than 25 bytes of mesh data\n");
      //flushprint();
      return(0);
   }
   ...
   if(gpar.printflag == PRINT_VERBOSE)
   {
      DBUG( printf( "SEND data to mesh\n");)
      DBUG( printf( "  Set [4][5] to byte counts\n");)
      DBUG( printf( "  Set [11].. to %d data bytes\n",count);)
   }
...
struct cticdata *cticalloc(int ndevice)
{
   ...
   while((*cpp)->type == CTIC_ACTIVE)
   {
      ...
      if(j >= 255)
      {
         printf( "Out of characteristic storage\n");
         return(&cticnull);
      }
   }
...
int devalloc()
{
   ...
   if(dn >= NUMDEVS)
   {
      printf( "Out of NUMDEVS device storage\n");
      return(-1);
   }
...
{    
   ...
   printf( "Scanning for LE devices - 10 seconds..\n");
   //flushprint();


   DBUG( printf( "Enable LE scan\n");)
   sendhci(lescanonf,0); 
   if(statusok(0,lescanonf) == 0)
   {
      printf( "Scan on failed\n");
      return(0);
   }

   readhci(0,0,IN_LESCAN,10000,0);  // may be multiple scan replies                

   DBUG( printf( "Disable LE scan\n");)
   sendhci(lescanoff,0);
   statusok(0,lescanoff);  
   ...
   do
   {
      ...
      {     
         printf( "Found %d unknown devices\n",newcount);
         popins();
         return(1);
      }
      ...
      {  // each response
                     ...
                     while(k < rp[j]-1)
                     {
                        if((k % 2) != 0)
                        {
                           if(rp[j+2+k] == 0xFE && rp[j+1+k] == 0xAA)
                              printf( "      FEAA Eddystone beacon\n");
                           else if(rp[j+2+k] == 0xFD && rp[j+1+k] == 0x6F)
                              printf( "      FD6F Contact tracing\n");
                           else
         ...
         if(ndevice >= 0)
         {
            printf( "    Known device %d %s\n",dev[ndevice]->node,dev[ndevice]->name);
            if(dev[ndevice]->type == BTYPE_ME && dev[ndevice]->node >= 1000)
               printf( "    Add to devices.txt with node < 1000 to authorise mesh packets\n");
            if(dev[ndevice]->type != BTYPE_LE && dev[ndevice]->type != BTYPE_ME)
               printf( "    But not listed as LE or Mesh\n");
         }
         else
         {  // not already stored
            if(type == BTYPE_ME)  // auto detect mesh device
            {
               printf( "    SECURITY NOTE - To receive mesh packets from this device\n");
               printf( "                    add to devices.txt with node < 1000\n");
            }
...
int newnode()
{
   ...
   printf( "Failed to find free node number\n");
   //flushprint();
   return(0);    
}
...
int device_info(int mask)
{
   ...
   for(n = 0 ; devok(n) != 0 ; ++n)
   {
      ...
      if(flag != 0)
      {
         ...
         if(n == 0 && dp->node != 0)
            printf( "%d  Local (%s)  ",dp->node,dp->name);        
         else
         {
            printf( "%d  %s  ",dp->node,dp->name);
         ...
         if(dp->type == BTYPE_LE && (dp->leaddtype & 1) != 0)
            printf( " Random - may change");

         if((dp->type & BTYPE_CL) != 0 && dp->pincode[0] != 0)
            printf( " PIN=%s",dp->pincode);    

         if((dp->type & BTYPE_CL) != 0 && dp->rfchan != 0)
            printf( " Channel=%d",dp->rfchan);
   ...
   for(n = 0 ; n < xn ; ++n)
   {
      ...
      {       
         ...
         if(j < count)
         {
            ...
            printf( "%d - %s",dev[j]->node,dev[j]->name);
         }
      }    
...
void mesh_server(int(*callback)())
{
   ...
   printf( "Mesh server listening (x = stop server)\n");
   //flushprint();
   do
   {
   ...
   if(read_error() == ERROR_KEY)
      printf( "Key press stop server\n");
   else if(read_error() == ERROR_FATAL)
      printf( "Fatal error stop server\n");
...
int le_server(int(*callback)(int clientnode,int operation,int cticn),int timerds)
{
   ...
   printf( "Listening for LE clients to connect (x=stop server)\n");
   ...
   do
   {
      ...
      {   
         ...
         if(op == LE_DISCONNECT)
            DBUG( printf( "%s has disconnected\n",dp->name);)
         else if(op == LE_CONNECT)
         {
            DBUG( printf( "%s has connected\n",dp->name);)
            mesh_on(); 
         }
   ...
   {
      printf( "LE Server disconnecting\n");
...
int node_server(int clientnode,int (*callback)(int clientnode,char *buf,int count),char endchar)
{
   ...
   {     
      printf( "Listening for %s to connect (x=cancel)\n",dp->name);
      //flushprint();
      do
      {
   ...
   printf( "Connected OK\n",dp->name);
   printf( "Waiting for data from %s (x = stop server)\n",dp->name);
...
int classic_server(int clientnode,int (*callback)(int clientnode,char *buf,int count),char endchar,int keyflag)
{
   ...
   DBUG( printf( "Set event mask\n");)
   sendhci(eventmask,0);
   statusok(0,eventmask);

   DBUG( printf( "Set simple pair mode on\n");)
   ...
   {     
      ...
      {     
         ...
         {
            tryflag = 0;
            printf( "  The remote device has connected and then disconnected.\n");
            printf( "  This may be OK - for example when pairing, which may have worked.\n"); 
            printf( "  But if pairing/connect failed, press k to change key option.\n");
            printf( "  If that fails, unpair on the remote device and try again. If pairing\n");
            printf( "  has worked, or if no further operation is expected, press x to stop\n");
            printf( "Listening for %s to connect (k=change key option x=stop server)\n",dp->name);
            //flushprint();
         }
      ...
      if((dp->conflag & CON_RF) == 0)
      {
         ...
         if(key == 'k')
         {
            ...
            if((dp->linkflag & KEY_ON) == 0)  
               printf( "Will not send a link key\n");
            else
               printf( "Will send a link key\n");
            //flushprint();
         }
   ...
   {
      if(read_error() == ERROR_KEY)
         printf( "Key press - stopping server\n");
      else if(read_error() == ERROR_FATAL)
         printf( "Fatal error - stopping server\n");
      else if(read_error() == ERROR_DISCONNECT)
         printf( "%s has disconnected - stopping server\n",dp->name);
   }
...
int classicserverx(int ndevice)
{
   ...
   DBUG( printf( "GOT Connect request (Event 4)\n");)
   DBUG( printf( "SEND Accept connection\n");)  
   ...
   {
      DBUG( printf( "Waiting for connect request\n");)
      dp->conflag = 0;
      return(0);
   }
   ...
   {  // probably just paired or read SDP then disconnected   
      DBUG( printf( "Waiting for connect RFCOMM request\n");)
...
void replysdp(int ndevice,int in,char *uuid,char *name)
{
   ...
   DBUG( printf( "GOT SDP data request\n");)
   ...
   if(insdat[in] == 0x02)
   {  // handle request
      DBUG( printf( "SEND handle that matches UUID\n");)
      if(uuidflag < 0)
      {
         DBUG( printf( "  No UUID match - send no data reply\n");)
   ...
   {  // 04/06 aid request
      ...
      {
         DBUG( printf( "SEND no data reply\n");)
         ssarep = ssaaidfail;
      }
      else  
      {
         DBUG( printf( "SEND SDP reply\n");)
         for(n = 0 ; n < 21 ; ++n)
...
int leconnect(int ndevice)
{
   ...
   DBUG( printf( "SEND LE connect to %s\n",dp->name); )
   ...
   {    // IN_LEHAND not saved to stack
      if(dp->type == BTYPE_LE || (dp->type == BTYPE_ME && dp->lecflag != 0))
         DBUG( printf( "Connect OK as LE client\n");) 
      else    
         printf( "Connect OK as NODE client\n");
      DBUG( printf( "Handle = %02X%02X\n",dp->dhandle[1],dp->dhandle[0]); )
   ...
   printf( "Connect Failed - no handle\n");
   if(dp->type == BTYPE_LE && (dp->leaddtype == 0 || (dp->leaddtype & 1) != 0))
   {
      if(dp->leaddtype == 0)   // on user list - may be random
      {
         printf( "This device may already be connected to another host.\n");
         printf( "This device may have a random board address which changes\n");
      }
      else                     // found by scan and is random
         printf( "This device has a random board address which may have changed\n");

      printf( "Scan for LE devices to find the current address\n");
   }
   //flushprint();
   return(0);      
}
...
int disconnectdev(int ndevice)
{
   ...
   if((dp->conflag & CON_RF) != 0)  // RFCOMM
   {
      if((dp->conflag & CON_SERVER) == 0) 
         printf( "Remote device may think it is still connected\n");
      DBUG(printf( "SEND Close RFCOMM request (opcode 53)\n"); )
      sendhci(closereqrf,ndevice);
      dp->conflag &= ~CON_RF;  
      readhci(ndevice,IN_RFUA,0,gpar.timout,gpar.toshort);
      if(findhci(IN_RFUA,ndevice,INS_POP) >= 0)
         DBUG( printf( "GOT UA reply (opcode 73)\n");)
   }

   if((dp->conflag & CON_CH0) != 0)  // CONTROL
   {
      DBUG( printf( "SEND Close CONTROL request (opcode 53)\n");)
   ...
   {
      DBUG( printf( "SEND Close connection\n");)
      ...
      if(dp->conflag == 0)
         DBUG( printf( "GOT Disconnected OK (Event 05)\n");)
      else
         DBUG( printf( "No disconnect OK (Event 05)\n");)
...
int closehci()
{
   ...
   if(gpar.hci <= 0)
   {
      DBUG( printf( "HCI not connected\n");)
      retval = 0;
   }
   else
   {  
      DBUG( printf( "HCI closed\n");)
      close(gpar.hci);
      gpar.hci = -1;
      retval = 1;
   }
...
int writecticx(int node,int cticn,unsigned char *data,int count,int notflag,int (*callback)())
{
   ...
   {  // local
      ...
      if(cp->type != CTIC_ACTIVE)
      {
         printf( "Invalid local characteristic index in write_ctic\n");
         //flushprint();
         return(0);
      }
      ...
      {
         printf( "Characteristic has no notify permission\n");
         //flushprint();
         return(0);
      }
      ...
      {  // search all other devices for notify
         ...
         {  // device n is connected as LE client
            // send notification
            DBUG( printf( "Send %s notification to %s\n",cp->name,dp->name);) 
   ...
   {
      printf( "%s not connected as LE server\n",dp->name);
      //flushprint();
      return(0);
   }
   ...
   if(cp->type != CTIC_ACTIVE)
   {
      printf( "Invalid node or characteristic index in write_ctic\n");
      //flushprint();
      return(0);
   }
   ...
   if(notflag == 0 && (cp->perm & 0x0C) == 0)
   {
      if(cp->perm == 0)
      {
         printf( "Write permit not known. Guessing = no ack\n");
         printf( "Set in device info or Read services (find_ctics)\n");
         //flushprint();
      }
      else
      {
   ...
   if(cp->chandle == 0)
   {
      printf( "No handle. Read characteristic or Read services to find\n");
      printf( "Read services (find_ctics) if write permit is needed and not known\n");
      //flushprint();
      return(0);
   }
   ...
   {
      printf( "Must specify byte count\n");
      //flushprint();
      return(0);
   }
   ...
   {
      locsize = LEDATLEN;
      printf( "Warning - can only write %d bytes to %s\n",LEDATLEN,cp->name); 
   }
   ...
   if(notflag == NOTIFY_ENABLE || notflag == NOTIFY_DISABLE)
   {
      if((cp->perm & 0x30) == 0)
      {
         printf( "Characteristic does not have notify/indicate permission\n");
         //flushprint();
         return(0);
      }
      ...
      if(notflag == NOTIFY_ENABLE)
      {
         DBUG( printf( "Enable notify/indicate for handle %04X\n",chandle);)
         data[0] = 1;  // enable notify
         cp->callback = callback;
      } 
      else
      {
         DBUG( printf( "Disable notify/indicate for handle %04X\n",chandle);)
         data[0] = 0;
         cp->callback = NULL;
      }
      data[1] = 0;
   ...
   if(gpar.printflag == PRINT_VERBOSE)
   {
      DBUG( printf( "Write %s %s =",dp->name,cp->name);)    
      for(n = 0 ; n < locsize ; ++n)
         printf( " %02X",cmd[n+12]);
      printf( "\n");
      DBUG( printf( "SEND write LE characteristic\n");)
      DBUG( printf( "  Set [3][4] [5][6] packet lengths\n");)
      DBUG( printf( "  Set [9] opcode %02X\n",cmd[9]);)
      DBUG( printf( "  Set [10][11] characteristic handle %04X\n",chandle);)
      DBUG( printf( "  Set [12].. %02X data bytes\n",locsize);)
      //flushprint();
   }
...
int read_ctic(int node,int cticn,unsigned char *data,int datlen)
{
   ...
   {  // local
      ...
      if(cp->type != CTIC_ACTIVE)
      {
         printf( "Invalid local characteristic index in read_ctic\n");
         //flushprint();
         return(0);
      }
      retval = 0;
   ...
   {  // use dev[]->ctic[cticn] info
      ...
      if(cp->type != CTIC_ACTIVE)
      {
         printf( "Invalid characteristic index in read_ctic\n");
         //flushprint();
         return(0);
      }
   ...
   DBUG( printf( "Read LE characteristic\n");)

   if(flag == 0)
   {                // set characteristic handle
      cmd[10] = (unsigned char)(chandle & 0xFF);
      cmd[11] = (unsigned char)((chandle >> 8) & 0xFF);                      
      DBUG( printf( "  Set [10][11] handle %02X%02X\n",cmd[11],cmd[10]);)
   }
   else
   {
      cmd[10] = 1;  // start handle = 1
      cmd[11] = 0;
      DBUG( printf( "  Set [14].. UUID %02X..\n",cmd[14]);)
   }
   ...
   if(n >= 0) 
   {
      ...
      {  // read OK
         ...
         else
         {
            ...
            if(retval > datlen-1)
            {
               printf( "  Need larger buffer in read_ctic\n");
...
int sendhci(unsigned char *s,int ndevice)
{
   ...
   {  // will need board address, handle etc.  
      ...
      {          
         ...
         DBUG( printf( "  Set [%d][%d] handle %02X %02X\n",n,n+1,dp->dhandle[0],dp->dhandle[1]);)
      }
      ...
      {  //  [4].. except LE open 0D 20
         ...
         DBUG( printf( "  Set [%d].. board address reversed %02X..%02X\n",n0,dp->baddr[5],dp->baddr[0]);)  
      ...
      {    
         ...
         {  // always psm 3
            cmd[7] = dp->dcid[0];  
            cmd[8] = dp->dcid[1];
            DBUG( printf( "  Set [7][8] remote L2CAP channel %02X %02X\n",cmd[7],cmd[8]);)
         }
         ...
         if((s[2] & S2_ADD) != 0)
         {
            ...
            if((dp->conflag & CON_SERVER) != 0)
               ...
               cmd[9] += 3;  // client  
            DBUG( printf( "  Set [9] RFCOMM address %02X\n",cmd[9]);)
         }

         if((s[3] & S3_ADDX) != 0)
         {
            cmd[9] = (dp->rfchan << 3) + 1;   
            DBUG( printf( "  Set [9] CONTROL address %02X\n",cmd[9]);)
         } 
         ...
         if((s[3] & S3_DCID1) != 0)
         {
            cmd[13] = dp->dcid[n0];
            cmd[14] = dp->dcid[n0+1];
            DBUG( printf( "  Set [13][14] remote L2CAP channel %02X %02X\n",cmd[13],cmd[14]);)
         }
         ...
         if((s[3] & S3_SCID2) != 0)
         {
            cmd[15] = dp->scid[n0];
            cmd[16] = dp->scid[n0+1];
            DBUG( printf( "  Set [15][16] local L2CAP channel %02X %02X\n",cmd[15],cmd[16]);)
         }
         ...
         if((s[3] & S3_SCID1) != 0)
         {
            cmd[13] = dp->scid[n0];
            cmd[14] = dp->scid[n0+1];
            DBUG( printf( "  Set [13][14] local L2CAP channel %02X %02X\n",cmd[13],cmd[14]);)
         }
         ...
         if((s[3] & S3_DCID2) != 0)
         {
            cmd[15] = dp->dcid[n0];
            cmd[16] = dp->dcid[n0+1];
            DBUG( printf( "  Set [15][16] remote L2CAP channel %02X %02X\n",cmd[15],cmd[16]);)
         }

         if((s[3] & S3_DLCIPN) != 0)
         {
            cmd[14] = dp->rfchan << 1;
            DBUG( printf( "  Frame size at [18] = %02X%02X\n",cmd[19],cmd[18]);)
            DBUG( printf( "  Set [14] DLCI %02X\n",cmd[14]);)
         }
         ...
         if((s[2] & S2_SDP) != 0)
         {
            ...
            DBUG( printf( "  Set [7][8] remote psm 1 channel\n");)
            DBUG( printf( "  Set [10][11] id %02X%02X\n",cmd[10],cmd[11]);)   
         }

         if((s[3] & S3_ADDMSC) != 0)
         {
            cmd[14] = (dp->rfchan << 3) + 3;
            DBUG( printf( "  Set [14] RFCOMM address %02X\n",cmd[14]);)
         }
   ...
   if(gpar.printflag == PRINT_VERBOSE)
   {
      if(cmd[0] == 2)
      {
         ...
         if(chan == 1)
         {
            DBUG( printf( "< L2CAP 0001 Opcode:id = %02X:%02X   ",cmd[9],cmd[10]);)
            if(cmd[9] == 2 || cmd[9] == 4 || cmd[9] == 6 || cmd[9] == 10)
               DBUG( printf( "Expects reply %02X:%02X\n",cmd[9]+1,cmd[10]);)
            else if(cmd[9] == 3 || cmd[9] == 5 || cmd[9] == 7  || cmd[9] == 11)
               DBUG( printf( "is reply to %02X:%02X\n",cmd[9]-1,cmd[10]);)
            else
               DBUG( printf( "\n");)              
         }
         else if(chan == 4)
            DBUG( printf( "< L2CAP 0004 Opcode = %02X\n",cmd[9]);)       
         else if(chan >= 0x0040)
         {
            if((dp->conflag & CON_PSM1) != 0 && cmd[7] == dp->dcid[2])
               DBUG( printf( "< SDP operation\n");)
            else if(cmd[9] > 3)
            {
               DBUG( printf( "< RFCOMM Address:Opcode = %02X:%02X",cmd[9],cmd[10]);)
               if(cmd[10] == 0xEF)
                  DBUG( printf( "  Send serial data\n");)
               else
                  DBUG( printf( "\n");)
            }
            else
               DBUG( printf( "< CONTROL Address:Opcode = %02X:%02X\n",cmd[9],cmd[10]);)
         }
         else
            DBUG( printf( "< ? Opcode %02X\n",cmd[9]);)
      }
      else if(cmd[0] == 1)
         DBUG( printf( "< HCI OGF=%02X OCF=%02X\n",(cmd[2] >> 2) & 0x3F,((cmd[2] << 8) & 0x300) + cmd[1]);)
      else 
         DBUG( printf( "< Unknown");)
...
int statusok(int flag,char *cmd)
{
   ...
   do
   {
      ...
      if(n >= 0 && insdatn[1] == cmd[PAKHEADSIZE+1] && insdatn[2] == cmd[PAKHEADSIZE+2]) 
      {
         ...
         if(insdatn[3] == 0)  // status = OK
         {      
            DBUG( printf( "STATUS OK\n");)
            retval = 1;
         }
         else if(!(insdatn[1] == 0x0A && insdatn[2] == 0x20))   // not mesh on/off
            DBUG( printf( "STATUS failed OGF %02X OCF %02X\n",insdatn[2] >> 2,insdatn[1]);)   
      }
   }
   while(n >= 0 && repflag == 0);

   if(repflag == 0)
      DBUG( printf( "STATUS - no reply\n");)
...
int readhci(int ndevice,long long int mustflag,long long int lookflag,int timout,int toshort)
{
   ...
   if(level > 8)
   {
      printf( "ERROR - Notify callback has spawned too many nested reads\n"); 
      //flushprint();
      return(0);
   }
   ...
   do   
   {
         ...
         {         
            do       // read block of data - may be less than or more than one line
            {
               ...
               {    // nothing read and timed out - normal exit route                         
                  if(len > 0 || blen > 0)
                     printf( "Exit with partial reply\n");
                  ...
                  else
                  {
                     if(mustflag != IN_DATA && mustflag != IN_CONREQ && lesflag == 0)
                        DBUG( printf( "Timed out waiting for expected packet\n");) 
      ...
      if(buf[0] == 4)    // HCI events
      {
         ...
         {    
            ...
            else   // LE connected
            {
               ...
               {
                  if(devicen == 0)
                     printf( "Unknown device %s connected - rejected\n",baddstr(buf+n0,1));
                  else
                     printf( "Unwanted device %s connected - rejected\n",dev[devicen]->name);
         ...
         else if(gotflag == IN_CLHAND)
         {
            ...
            {       
               ...
               {  // is waiting for this connection
                  ...
                  dp->dhandle[1] = buf[5];    
                  dp->conflag |= CON_HCI;  
                  DBUG( printf( "GOT Open OK (Event 03) with handle = %02X%02X\n",buf[5],buf[4]);)
                  doneflag = 1;  
                  timstart = timems(TIM_RUN);
                  timendms = toshort;
               }
               ...
               {
                  printf( "Unknown device %s connected - rejecting..\n",baddstr(buf+n0,1));
            ...
            if(chan == 4)    // LE or mesh
            {
               ...
               else
               {
                  ...
                  {  // even opcode = request from server - let odd opcodes go to ATTDAT 
                     ...
                     if(buf[9] == 0x02)
                     {  // MTU exhange
                        DBUG( printf( "SEND MTU exchange reply\n");)
                        sendhci(lemtu,devicen);
                     }
                     else 
                     {
                        DBUG( printf( "%s is requesting attributes - fob it off\n",dev[devicen]->name);)
         ...
         {        
            if(gotflag == IN_DATA)
            {
               if(firstpacket == 0)   // extra data
               {
                  ...
                  DBUG( printf( "Following packet [3][4] = %04X extra bytes from [5]..\n",datlen);)
                  datp = buf+5;
                  xwantlen -= datlen;
                  nxx = dev[devicen]->nx;
      ...
      {         // only print le scan reply gotflag == LE_SCAN or meshflag = 1
         ...
         {
            if(xprintflag == 1)
               DBUG( printf( "> Node data - first packet\n");)
            else
               DBUG( printf( "> Extra bytes - add to previous data\n");)
         }
         else if(b0 == 4)
         {
            DBUG( printf( "> ");)
            printf( "Event %02X =",buf[1]);
            ...
            if(buf[1] == 0x0F && buf[3] != 0)
            {
               ...
               if(k > 69)
                  k = 0;  // no error0f text
               DBUG( printf( "*** FAIL *** CMD %02X %02X Error %02X %s\n",buf[5],buf[6],buf[3],error0f[k]);)
            }       
         }
         else if(b0 == 2)
         {     
            if(chan == 1)
            {
               DBUG( printf( "> L2CAP 0001 Opcode:id = %02X:%02X",buf[9],buf[10]);)
               if(buf[9] == 2 || buf[9] == 4 || buf[9] == 6 || buf[9] == 10)
                  DBUG( printf( "  Must send reply %02X:%02X\n",buf[9]+1,buf[10]);)
               else if(buf[9] == 3 || buf[9] == 5 || buf[9] == 7  || buf[9] == 11)
                  DBUG( printf( "  is reply from %02X:%02X\n",buf[9]-1,buf[10]);)
               else
                  printf( "\n");
            }
            else if(chan == 4)
            ...
            else if(chan >= 0x40)
            {
               ...
               {
                  DBUG( printf( "> RFCOMM Address:Opcode %02X:%02X",buf[9],buf[10]);)
                  if(crflag != 0)       // not control channel
                     DBUG( printf( "   received serial data");)   // serial data
               }   
               else
               {
                  DBUG( printf( "> CONTROL Address:Opcode %02X:%02X",buf[9],buf[10]);)
               }  
               if( (buf[9] & 2) == 0 && buf[10] == 0x53)  // close from remote
                  DBUG( printf( "   close request. Must send %02X:73 UA reply\n",buf[9]);)
               else
                  DBUG( printf( "\n");)         
            }
            else
            {
               DBUG( printf( "> L2CAP %04X ? = ",chan);)
         ...
         if(xflag != 0)
         {
            DBUG( printf( "Need an extra %04X bytes\n",xwantlen);)
         }
      }   // end print
      ...
      if(crflag != 0)
      {
         DBUG( printf( "  Has used a credit. Number remaining = %d\n",condp->credits);)  
         if(condp->credits < 3)
...
void immediate(long long lookflag)
{
   ...
   while(1)
   {
      ...
      {
         id = insdat[n+1];
         DBUG( printf( "GOT L2 request config - id %02X\n",id);)
         j = insdat[n+4];
         if(j == 0x41 || j == 0x42)
            dp->psm = 1;
         else
         ...
         {  // server  (already done if client)
            dp->id = 5;
            DBUG( printf( "SEND L2 config request. Choose id %02X\n",dp->id);)
            //flushprint();
            sendhci(figreq,devicen); 
         }

         dp->id = id;
         DBUG( printf( "SEND L2 config reply\n");)
      ...
      else if(gotflag == IN_L2ASKINFO)           
      {
         j = insdat[n+4];
         dp->id = insdat[n+1];
         DBUG( printf( "GOT ask info (opcode 0A) type %d\n",j);)
         if(j == 2)
         {
            DBUG( printf( "SEND info reply type 2\n");)
            sendhci(inforeply2,devicen);
         }    
         else if(j == 3)
         {
            DBUG( printf( "SEND info reply type 3\n");)
      ...
      {
         dp->id = insdat[n+1];
         DBUG( printf( "GOT ask echo (opcode 08)\n");)
         DBUG( printf( "SEND echo reply\n");)
      ...
      else if(gotflag == IN_DISCH)
      {
         ...
         {   // should be RFCOMM channel or CONTROL 0 channel
            DBUG( printf( "GOT Disconnect channel %d request (opcode 53)\n",ch);)
            DBUG( printf( "SEND UA reply (opcode 73)\n");)
            DBUG( printf( "  SET [9] Same address\n");)
         }
      ...
      else if(gotflag == IN_CONCHAN)
      {
         ...
         if(gpar.printflag == PRINT_VERBOSE)
         {
            DBUG( printf( "GOT open channel %d (opcode 3F)\n",ch);)
            DBUG( printf( "SEND UA reply\n");)
            DBUG( printf( "  Set [9] Same address\n");)
         }
         uareply[PAKHEADSIZE+9] = insdat[n];
      ...
      else if(gotflag == IN_RFCHAN)
      {
         ...
         DBUG( printf( "%s is trying to connect on channel %d frame size %02X%02X\n",dp->name,dp->rfchan,insdat[n+10],insdat[n+9]);)

         for(j = 0 ; j < 9 ; ++j)
            pnreply[PAKHEADSIZE+13+j] = insdat[n+4+j];

         DBUG( printf( "SEND PN reply\n");)
      ...
      {
         j = insdat[n+3];
         DBUG( printf( "GOT MSC %02X\n",j);)
         DBUG( printf( "SEND MSC reply\n");)
      ...
      else if(gotflag == IN_L2ASKCT)
      {
         ...
         {  // only allow server psm 1/3 
            DBUG( printf( "  GOT L2 connect request. Fob it off\n");)
         ...
         else
         {
            ...
            DBUG( printf( "GOT L2 connect request psm %d channel %02X%02X\n",insdat[n+4],insdat[n+7],insdat[n+6]);)  
            ...
            DBUG( printf( "SEND connect reply\n");)
      ...
      {
         DBUG( printf( "GOT link request (Event 17)\n");) 

         if((dp->linkflag & KEY_ON) == 0)
         {
            DBUG( printf( "GOT 17 Send neg reply\n");) 
            DBUG( printf( "SEND link request neg reply\n");)  
            sendhci(linkreply,devicen);
         }
         else      
         {
            DBUG( printf( "GOT 17 Send key\n");) 
            DBUG( printf( "SEND link key\n");) 
            for(j = 0 ; j < 16 ; ++j)
      ...
      {
         DBUG( printf( "GOT link key (Event 18)\n");)  
         for(j = 0 ; j < 16 ; ++j)
      ...
      {
         DBUG( printf( "GOT IO Cap reply (Event 32) I/O=%d Auth=%d\n",insdat[n+6],insdat[n+8]);)  
      }
      else if(gotflag == IN_PAIRED)
      {
         DBUG( printf( "GOT Event 36 status %d\n",insdat[n]);) 
         if(insdat[n] == 0)
            DBUG( printf( "Paired with %s\n",dp->name);)
         else
      ...
      else if(gotflag == IN_IOCAPREQ)
      {
         ...
         if(gpar.printflag == PRINT_VERBOSE) 
         {  
            DBUG( printf( "GOT IO capability request (Event 31)\n");)
            DBUG( printf( "SEND IO capability reply %d\n",iocapreply[PAKHEADSIZE+10]);)
         }   
      ...
      {      
         DBUG( printf( "GOT PIN request (Event 16)\n");)
         if(dp->pincode[0] == 0)
         {
            //flushprint();
         ...
         {
            printf( "Using PIN=%s from device info\n",dp->pincode);
            j = strlen(dp->pincode);
         }

         pincode[PAKHEADSIZE+10] = j;   
         strcpy(pincode+PAKHEADSIZE+11,dp->pincode);
         DBUG( printf( "SEND PIN code\n");)
         DBUG( printf( "  Set [10] PIN length\n");)
         DBUG( printf( "  Set [11] PIN = %s\n",dp->pincode);)

         sendhci(pincode,devicen);
      }     
      else if(gotflag == IN_PASSREQ)
      {      
         DBUG( printf( "GOT passkey request (Event 34)\n");)
         //flushprint();
         ...
         DBUG( printf( "SEND Passkey\n");)
         DBUG( printf( "  Set [10] = passkey\n");)
      ...
      {
         DBUG( printf( "GOT User confirm request with passkey (Event 33)\n");)
         DBUG( printf( "SEND User confirm reply\n");)
      ...
      {  // LE notify (1B) or indicate (1D)
         ...
         if(getout != 1)
            DBUG( printf( "Unknown LE characteristic notified\n");)
         else
         {
            if (gpar.printflag == PRINT_VERBOSE)
            { 
               DBUG(printf( "%s %s notify =",dp->name,cp->name); )

               for(j = 0 ; j < bn ; ++j)
                  printf( " %02X",insdat[n+3+j]);
               printf( "\n");
            }
            if(cp->callback != NULL)
         ...
         DBUG( printf( "Unrecognised immediate\n");)
...
void leserver(int ndevice,int count,unsigned char *dat)
{
   ...
   DBUG( printf( "GOT LE server opcode %02X from %s\n",dat[0],dev[ndevice]->name);) 
   //flushprint();
   ...
   {  // read/write
         ...
         {    
            ...
            {  // write  12 ack  52 no ack
               if(notflag == 0)
               {
                  if((cp->perm & 0x0C) == 0)
                  {   // 8 ack  4 no ack
                     DBUG( printf( "Write not permitted\n");)
                     errcode = 3;  // write not permit
                  }
                  else
                  {
                     DBUG( printf( "Received characteristic %s\n",cp->name);)
                     write_ctic(node,cticn,dat+3,count-3);
                     if(dat[0] == 0x12)
                     {  // no check cp->perm & 8 write with ack
                        DBUG( printf( "Send acknowledgement\n");)
               ...
               else  // notify descriptor
               {
                  cp->notify = dat[3];
                  if(cp->notify == 0)
                     DBUG( printf( "%s notify disable\n",cp->name);)
                  else
                     DBUG( printf( "%s notify enable\n",cp->name);)
                  if(dat[0] == 0x12)
                  {
                     DBUG( printf( "Send acknowledgement\n");)
                     sendhci(leack,ndevice);  // send ack
            ...
            {  // read
               if(notflag == 0)
               {
                  if((cp->perm & 2) == 0)
                  {
                     DBUG( printf( "Read not permitted\n");)
               ...
               {            
                  ...
                  DBUG( printf( "SEND characteristic %s\n",cp->name);)
      ...
      if(flag == 0)
      {
         printf( "%s trying to read/write invalid handle %04X\n",dev[ndevice]->name,handle);
         errcode = 1;
      }
   ...
   else if(dat[0] == 0x04)
   {
      ...
      DBUG( printf( "Read attribute info for handles %04X to %04X\n",start,end);)
      ...
      if(flag == 0)
      {
         DBUG( printf( "Attribute not found\n");)
         errcode = 0x0A;  // attrib not found
      }
      else
      {
         DBUG( printf( "SEND reply opcode 05 for handle %02X%02X\n",s[12],s[11]);)         
   ...
   { 
      ...
      uuidtype = count - 5;
      DBUG( printf( "Read attribute info for handles %04X to %04X and\n  UUID ",start,end);)
      for(n = 0 ; n < uuidtype ; ++n)
         DBUG( printf( "%02X",dat[uuidtype+4-n]);)
      printf( "\n");
      ...
      if(flag == 0)
      {
         DBUG( printf( "Attribute/UUID not found\n");)
      ...
      else
      {
         ...
         DBUG( printf( "SEND reply opcode %02X for handle %02X%02X\n",s[9],s[12],s[11]);)
   ...
   { // request not supported
      DBUG( printf( "Opcode not supported\n");)
      if(dat[0] <= 0x20)
         errcode = 0x06;
   ...
   if(errcode != 0)
   {
      ...
      DBUG( printf( "SEND error code %02X\n",errcode);) 
...
void rwlinkey(int rwflag,int ndevice)
{
   ...
   {  // read
      if(count < 0)
      {
         ...
         if(n != 0)
         {
            printf( "Read link.key file failed\n");
            return;
         }
      }
   ...
   {  // write   
      ...
      {
         printf( "Too many link keys - delete link.key file to reset\n");
         return;
      }
   ...
   {  // each response
      ...
      {  // is a mesh packet
         ...
         {  // not known - reject
            if(rejflag == 0)
            {  // print only once 
               printf( "GOT mesh packet from unknown device %s - rejected for\n",baddstr(rp+2,1));
               printf( "    security. Add to devices.txt with node < 1000 to authorise\n");
               rejflag = 1;
            }
            return(1);
         }
         ...
         DBUG( printf( "GOT mesh packet %d from %s\n",index,dev[ndevice]->name);)  
...
int pushins(long long int typebit,int devicen,int len,unsigned char *s)
{
   ...
   {
      printf( "Serial buffer full - use read_all_clear()\n");
      return(-1);
   }
...
int hcisock()
{
   ...
   DBUG( printf( "Open HCI user socket\n");)

   // AF_BLUETOOTH=31
   DBUG( printf( "Open BTPROTO socket\n");)
   dd = socket(31, SOCK_RAW | SOCK_CLOEXEC | SOCK_NONBLOCK, BTPROTO_HCI);

   if(dd < 0)
   {
      DBUG( printf( "Socket open error\n");)
      //flushprint();
      return(0);
   }
   ...
   DBUG( printf( "Set HCI filter\n");)

   if(setsockopt(dd, SOL_HCI, HCI_FILTER, &flt, sizeof(flt)) < 0)
   {
      DBUG( printf( "HCI filter setup failed\n");)
      close(dd);
   }   

   DBUG( printf( "Bind to Bluetooth devid user channel\n");)
   ...
   DBUG( printf( "Set page/inquiry scan and timeouts = 10 secs\n");)
...
int openremotesdpx(int ndevice)
{
   ...
   printf( "Connecting to %s to read classic serial services...\n",dev[ndevice]->name);
...
unsigned char calcfcs(unsigned char *s,int count)
{
   ...
   DBUG( printf( "  Set [%d] FCS=%02X calculated for %d bytes from [9]\n",len-1,fcs,count);) 
   return(fcs);     
}
...
int clconnect0(int ndevice)
{
   ...
   {
      printf( "Invalid or not classic device\n");
      return(0);
   }
   ...
   if(retval == 0)
   { 
      DBUG( printf( "DISCONNECT\n");)
      disconnectdev(ndevice);  // sets conflags
      return(0);
   }

   DBUG( printf( "HCI Connected OK\n");)
...
int clconnectxx(int ndevice)
{
   ...
   DBUG( printf( "Open classic connection to %s\n",baddstr(dp->baddr,0));)
   ...
   do
   {
      DBUG( printf( "SEND Authentication request\n");)
      ...
      if(n < 0)
      {
         printf( "Authentication/PIN fail\n");
         if(tryflag == 0)
         {
            tryflag = 1;
            // flip KEY
            dp->linkflag ^= KEY_ON;
            if((dp->linkflag & KEY_ON) == 0)  
               printf( "Trying again with no link key..\n");
            else
               printf( "Trying again with link key..\n");
         }
         else
         {
            /*****
              ...
              dp->linkflag &= ~KEY_NEW;
              printf( "Link key may be unknown or invalid\n");
              printf( "On remote device - unpair or unpair and re-pair\n");
              }
              }
             *******/  
            return(0);
   ...
   DBUG( printf( "GOT Authentication/pair OK (Event 06)\n");)


   DBUG( printf( "SEND encrypt\n");)    
   ...
   DBUG( printf( "GOT Encrypt OK (Event 08)\n");)
...
int clservices(int ndevice,int flags,char *uuid)
{
   ...
   if(dp->conflag != 0)
   {
      printf( "%s must be disconnected and listening as a classic server to read serial channels\n",dp->name);
      return(-1);
   }
   ...
   {
      printf( "UUID not specified\n");
      return(-1);
   }

   clearins(0); 

   DBUG( printf( "Reading SDP database of %s\n",dp->name);)
   ...
   if(gpar.printflag == PRINT_VERBOSE)
   {
      DBUG( printf( "Find all UUID = %02X%02X\n",ssareq[PAKHEADSIZE+17],ssareq[PAKHEADSIZE+18]);)          
      DBUG( printf( "  Set [%d][%d] UUID\n",headsz+8,headsz+9);)   
   }
   ...
   do
   {
      ...
      if(headsz != 0)
         DBUG( printf( "  Set [3] [5] packet lengths\n");)
      cmd[3] = 24 + ncont;
      cmd[5] = 20 + ncont;

      //   DBUG( printf( "  Set [%d] id %02X\n",headsz+2,id);)
      //   cmd[11] = id;

      cmd[13] = 15 + ncont;   
      DBUG( printf( "  Set [%d] length %02X\n",headsz+4,cmd[13]);)
      if(ncont == 0)
      {
         DBUG( printf( "  Set [%d] continue bytes count = 0\n",headsz+19);)
         cmd[28] = 0;  // else filled in on previous reply
      }
      if(gpar.printflag == PRINT_VERBOSE)
      {
         if(flags == SRVC_FINDUUID2 || flags == SRVC_FINDUUID16 || flags == SRVC_FREECHANNEL)
            DBUG( printf( "SEND SSA request aid 0 to 4\n");)
         else
            DBUG( printf( "SEND SSA request all aid\n");)
      }
      ...
      if(n < 0)
      {
         DBUG( printf( "No SSA reply\n");)
         ncont = 0;
      }
      else
      {
         DBUG( printf( "GOT SSA reply\n");)
         dat = insdat + n; // reply starting at opcode 07
         if(dat[0] != 7)
            ncont= 0;  // error
         else
         {
            DBUG( printf( "[%d]... = packet lengths  [%d].. = SDP data\n",headsz+3,headsz+7);)
            ...
            if(sn != 0)
               DBUG( printf( "Remove continue bytes and add to previous data\n");)
            ...
            if(ncont > 32)
            {
               DBUG( printf( "**** Invalid ncont\n");)
               ncont = 32;
            }

            if(ncont > 0)
            {  // copy ncont+1 continue bytes to end of ssareq command cmd[28..] 
               DBUG( printf( "Last packet has ended with %d continue bytes\n",ncont);)
               DBUG( printf( "Need another SSA request with these bytes:\n");)
               ...
               DBUG( printf( "  Set [%d]..  count and continue bytes..\n  ",headsz+19);)
               for(k = 0 ; k <= ncont ; ++k)
               {
               ...
               DBUG( printf( "No continue bytes - done\n");)
         }
         popins();
      }
      //flushprint();
   ...
   {
      DBUG( printf( "No SDP data\n");)
      if(flags == SRVC_UUID)
         printf( "No matching SDP entries\n");
      return(-1);
   }

   if(sn > 0)
   {
      DBUG( printf( "Decode SDP info %d bytes start %02X %02X %02X end %02X %02X %02X\n",sn,sdat[0],sdat[1],sdat[2],sdat[sn-3],sdat[sn-2],sdat[sn-1]);)
      ...
      {     
         ...
         if(len > 0)
         {
            ...
            for(k = 0 ; k < len && k < 16 ; ++k)
               printf( "%02X",serv[n].uuid[k]);

            if(len == 2)
               printf( "  %s",uuidlist+finduuidtext((serv[n].uuid[0] << 8) + serv[n].uuid[1]));
            printf( "\n");

            if(ndevice == 0)   // classic SDP local info
               printf( "    Local SDP handle = %08X\n",serv[n].handle);
         }
      } 
      ++count;    
      //flushprint();
...
void printlocalchannels()
{
   ...
   printf( "      Serial2  UUID=1101\n");
   printf( "      Serial16  UUID=00001101-0000-1000-8000-00805F9B34FB\n") ;
   printf( "      %s  UUID=",custname); 
   for(n = 0 ; n < 16 ; ++n)
   {
      printf( "%02X",custuuid[n]);
      if(n == 3 || n == 5 || n == 7 || n == 9)
         printf( "-");
   } 
   printf( "\n");
...
int leservices(int ndevice,int flag,char *uuid)
{
   ...
   if(dev[ndevice]->conflag == 0)
   {
      printf( "%s must be connected as an LE server to read characteristics\n",dev[ndevice]->name);   
      return(-1);
   }
   ...
   printf( "Reading LE services from %s..",dev[ndevice]->name);
   ...
   {           // go through handles 0001...  until get opcode=01 no more response 
      ...
      DBUG( printf( "SEND read UUID (opcode 08)\n");)
      DBUG( printf( "  Set [10][11] starting ctic handle %02X%02X\n",cmd[11],cmd[10]);)
      DBUG( printf( "  Set [14][15] UUID %04X\n",locuuid);)
            ...
            {          
               ...
               {    // UUID = 2803 characteristics      
                  ...
                  { 
                     ...
                     if(chn >= SERVDAT-2)
                     {
                        DBUG( printf( "Run out of service memory\n");)  
                        getout = 2;
                     }        
               ...
               {  // not looking for 2803 characteristics - print data
                  printf( "Handle %04X =",lasth);
                  for(k = 0 ; k < len-2 ; ++k)
                     printf( " %02X",insdatn[n0+2+k]);
                  printf( "\n");               
               }
               ++count;   
   ...
   for(n = 0 ; ctic(ndevice,n)->type == CTIC_ACTIVE ; ++n)
   {
      cp = ctic(ndevice,n);
      if(cp->iflag == 0)
         printf( "WARNING does not have %s as listed in local devices.txt info\n",cp->name); 
   }
...
int printctics1(int ndevice)
{
   ...
   if(ctic(ndevice,0)->type != CTIC_ACTIVE)
   {
      if(dev[ndevice]->type == BTYPE_LE)
         printf( "     No characteristics - Read services to find\n");
      return(0);
   }

   printf( "   ctic\n");
   printf( "   index       LE Characteristics\n");
   ...
   for(k = 0 ; ctic(ndevice,k)->type == CTIC_ACTIVE ; ++k)
   {
      ...
      printf( " %s byte Permit %02X %s ",sizes,cp->perm,perms[pn]);

      if(cp->chandle == 0)
         printf( "Handle ? ");
      else
         printf( "Handle=%04X ",cp->chandle);

      if(ndevice == 0 && cp->size < 8) 
      {
         printf( "Value=");
         for(j = 0 ; j < cp->size && j < LEDATLEN ; ++j)
            printf( "%02X ",cp->value[j]); 
      }
...
int printctics0(int devicen,int flags)
{
   ...
   delflag = 10;
   printf( "ctic      LE characteristics\nindex\n"); 
   for(n = 0 ; n < xn ; ++n)
   {
      cp = ctic(devicen,vn[n]);
      printf( " %d  %s",vn[n],cp->name);
      ...
      if(flag != 0)
      {
         ...
         if(j < count)
         {
            ...
            printf( "%d  %s",jn,ctic(devicen,jn)->name);
         }
      }    
      ...
      if(serv[n].channel == 0x10000 && (serv[n].perm & 0x0E) != 0)
      {
         ...
         if((cp->perm & 2) != 0) 
         {
            if(read_ctic(dev[devicen]->node,cp->cticn,buf,64) == 0)
               printf( "  Error reading characteristic index %d\n",cp->cticn);
         } 
      } 
   }


   printf( "Characteristics saved to device info\n");
...
int connectpsm(int psm,int channel,int ndevice)
{
   ...
   if(gpar.printflag == PRINT_VERBOSE)
   {  
      DBUG( printf( "SEND connect L2CAP channel psm %d\n",psm);)   
      DBUG( printf( "  Choose local channel %02X%02X (Must be >= 0x0040). Choose id %d\n",dp->scid[1],dp->scid[0],dp->id);)   
      DBUG( printf( "  Set [13] psm %02X\n",psm);)
   }
   ...
   if(n < 0)  
   {
      printf( "NO L2 connect reply %d %d\n",okflag,count);
      return(0);
   }

   if(gpar.printflag == PRINT_VERBOSE) 
   {
      DBUG( printf( "GOT connect OK reply with remote channel\n");)
      DBUG( printf( "  Remote channel for following sends = %02X%02X\n",insdatn[5],insdatn[4]);)
   }
   ...
   if(n < 0)
   {
      printf( "NO L2 config reply\n");
      return(0);
   }

   DBUG( printf( "GOT L2 config reply\n");)


   popins();


   DBUG( printf( "Connect L2CAP psm %d now done OK\n",psm);)
...
int disconnectl2(int ndevice,int psm)
{
   ...
   dp->psm = psm;
   dp->id = 8;
   DBUG( printf( "SEND disconnect L2CAP channel. Choose id %02X\n",dp->id);)
   ...
   if(findhci(IN_L2REPDIS,ndevice,INS_POP) < 0)
      DBUG( printf( "Not seen L2CAP disconnect reply\n");)
   else
      DBUG( printf( "GOT L2CAP disconnect done reply\n");)
      ...
      for(repn = 0 ; repn < nrep ; ++repn)
      {
         ...
         if(ndevice >= 0)
         {
            ...
            if(dev[ndevice]->type != BTYPE_CL && dev[ndevice]->type != BTYPE_ME)
               printf( "   But not listed as Classic or Mesh\n"); 
         }
         else
         {     
            ndevice = devalloc();
            ...
            {   // no name
               ...
               else
               {
                  ...
                  printf( "   Unable to read name so set = %s\n",buf);
               }
            }
            else
            {  // got name
...
int decodedes(char *sin,int len,struct sdpdata *sdpp)  //  int level,int aidflag,int record,unsigned int callaid)
{
   ...
   do
   {
      ...
      if(sdpp->aid < 0 && s[0] == 0x09)  // found first aid - this is aid level 
      {
         ...
         DBUG( printf( " **** RECORD %d ****\n",sdpp->record);)
      } 

      type = s[0] >> 3;
      if(type > 8)
      {
         printf( "Invalid des type %d\n",type);
         return(0);
      }
      size = s[0] & 7;
      ++s;
      ...
      else   // single value - not a des     
      {
         ...
         if(locaidflag != 0)   // only at first level where 09
         {
            ...
            { 
               sdpp->aid = (s[0] << 8) + s[1];
               DBUG( printf( "aid = %04X",sdpp->aid);)
               prflag = 0;
               //   if(sdpp->aid == 0)
               //     printf( "  ***** RECORD %d **** ",sdpp->record);
            }
         ...
         {   // rfcomm channel
            DBUG( printf( "RFCOMM channel = %d",s[0]);)
            ...
            if(sdpp->uuidtype > 0 && sdpp->uuid != NULL)
            {
               if(sdpp->chdn < sdpp->servlen-2)
               {
                  ...
                  printf( "Need more channel entries\n");
...
int connect_node(int node,int channelflag,int channel)
{
   ...
   else if(type == BTYPE_LE || type == BTYPE_ME)
   {
      if(type == BTYPE_ME && channelflag == CHANNEL_LE)
         ...
         printf( "NOTE - should use: connect_node(node,CHANNEL_LE,0)\n");   
...
int clconnect(int ndevice,int channel,int channelflag)
{
   ...
   {
      printf( "Classic server must use CHANNEL_NEW or CHANNEL_STORED\n");
      return(0);
   }  
   ...
   {  // use previous rfchan
      if(dp->rfchan == 0)
      {
         printf( "No stored channel from device info or previous connect\n");
         return(0);
      }
   } 
   else 
...
void setcredits(int ndevice)
{
   ...
   if(gpar.printflag == PRINT_VERBOSE)
   {
      DBUG( printf( "SEND Top up credits\n");)
      DBUG( printf( "  Set [12] credits = %02X\n",dp->credits);)
   }
...
{  
   ...
   if(gpar.printflag == PRINT_VERBOSE)
   {
      DBUG( printf( "SEND Open CONTROL channel 0\n");)  //56
      DBUG( printf( "  Command address = 03\n");)
      DBUG( printf( "  Reply address = 01\n");)
      DBUG( printf( "  DLCI = 0\n");)
   }
   ...
   if(gpar.printflag == PRINT_VERBOSE)
   {
      DBUG( printf( "GOT UA reply (03 73)\n");)   // 58 
      DBUG( printf( "Open RFCOMM channel %02X obtained from remote SDP services database\n",dp->rfchan);)  //61
      DBUG( printf( "  Command address (channel<<3+3) = %02X\n",(dp->rfchan << 3) + 3);)
      DBUG( printf( "  Reply address   (channel<<3+1) = %02X\n",(dp->rfchan << 3) + 1);)
      DBUG( printf( "  DLCI (channelx2) = %02X\n",dp->rfchan << 1);) 
      DBUG( printf( "SEND PN CMD to set RFCOMM channel and parameter negotiation\n");)    // 59
   }
   ...
   {
      printf( "No PN RSP reply - probably server not listening on channel %d\n",dp->rfchan);
      return(0);
   }

   DBUG( printf( "GOT PN RSP reply (01 EF 81)\n");)  // 60

   popins();


   DBUG( printf( "SEND Open RFCOMM channel %02X\n",dp->rfchan);)  //61
   ...
   DBUG( printf( "Serial RFCOMM channel is now open for read/write\n");)
...
int write_node(int node,unsigned char *outbuff,int count)
{
   ...
   {
      printf( "%s not classic/node connected for write\n",dev[ndevice]->name);
      //flushprint();
      return(0);
   }  
   ...
   if(count > n)   
   {
      printf( "Write too many bytes (max %d)\n",n);
      //flushprint();
      return(0);
   }  
   ...
   {
      //  packet size=len+9      [3][4]=len + 4  [5][6]=len  [9]=data 
      DBUG( printf( "SEND %d bytes ATT data\n",count);)
   ...
   if(gpar.printflag == PRINT_VERBOSE)
   {
      ...
      DBUG( printf( "SEND DATA %d bytes = %s\n",count,buf);)
   }
   ...
   {  // 1 byte length
      ...
      datn = 12;  // for print
      DBUG( printf( "  Set [11] one length byte = %02X (length*2 + 1 to indicate 1 byte)\n",bs[11]);)   
   }
   else 
   {  // 2 byte length
      ...
      datn = 13;
      ++xlength;  // one longer
      DBUG( printf( "  Set [11][12] two length bytes = %02X %02X (length*2) (length >> 7)\n",bs[11],bs[12]);)   
   }
   ...
   bs[5] = (xlength+4) & 255;
   bs[6] = (xlength+4) >> 8;
   DBUG( printf( "  Set [3].. packet lengths %02X %02X %02X %02X\n",bs[3],bs[4],bs[5],bs[6]);)
   for(n = 0 ; n < count ; ++n)
      dat[n] = outbuff[n];  

   DBUG( printf( "  Set [%d].. %d data bytes %02X..\n",datn,count,dat[0]);)
   //flushprint();
...
int readserial(int *node,char *inbuff,int count,char endchar,int flag,int timeoutms)
{
   ...
   {
      printf( "Time out/key flags or count zero in read\n");
      //flushprint();
      *node = 0;
      return(0);
   }    
   ...
   {       // mesh or cl connected search
      ...
      if(clcon + meshcon == 0)
      {
         //    printf( "No connected devices in read\n");
         *node = 0;
         //    //flushprint();
         //    return(0);
      }       
   ...
   {   // specified device
      printf( "%s not classic/node connected for read\n",dev[ndevice]->name);
      //flushprint();
      *node = 0;
      return(0);
   }
   ...
   do  // until exit from within loop
   {
      do   // until no more IN_DATA on stack
      {
         ...
         {   // data on instack - may be on first entry
            ...
            if(devok(devicen) == 0)
               DBUG( printf( "GOT data from unknown device\n");)
            else
            {                
               DBUG( printf( "GOT data from %s\n",dev[devicen]->name);)   
            }
            ...
            do    // while still something on instack
            {
               ...
               {  // count is buffer size
                  printf( "Read exceeded buffer size\n");
                  popins();
                  timems(TIM_FREE);
      ...
      {
         DBUG( printf( "READ disconnect exit\n");) 
      ...
      if( (flag & EXIT_TIMEOUT) != 0)
      {
         ...
         {
            DBUG( printf( "Serial read time out\n");)
      ...
      if( (flag & EXIT_KEY) != 0)
      {
         if(readkey() == 'x')
         {
            DBUG( printf( "Serial read aborted by key press\n");)
...
int output_file(char *filename)
{
   ...
   printf( "All screen output saved to %s file\n",filename);
   fclose(prstream);
   //flushprint();
   return(1);
}
...
char *strtohexx(char *s,int slen,int *num)
{
   ...
   if(errflag != 0)
   {
      printf( "Hex format error - odd number of digits\n");
      //flushprint();
      if(num != NULL)
         *num = vn;
      return(val);
...
int addDevice( char *name, int type, char *bt_address, int node )
{
   int dn = devalloc();
   if(dn < 0)
      return -1;  // fatal alloc error

   struct devdata *dp = dev[dn];
   strcpy( dp->name, name );
   dp->type = type;

   int entries;
   char *data = strtohexx(bt_address,strlen(bt_address),&entries);
   if ( entries != 6 )
      return -1;
...
int addCtic( int dn, char *name, int perm, int handle, int size, int cticn )
{
   ...
   cp = cticalloc(dn); // get ctic pointer - may be cticnull=failed

   if(cp->type != CTIC_UNUSED)
      return(0);   // fatal alloc error

   //cp->uuidtype = 2;     // two byte UUID

   strcpy( cp->name, name );
   cp->perm    = perm;                  // PERMITs
   cp->chandle = handle;
   cp->size    = size;
   cp->cticn   = cticn;
   cp->type    = CTIC_ACTIVE;  // used
